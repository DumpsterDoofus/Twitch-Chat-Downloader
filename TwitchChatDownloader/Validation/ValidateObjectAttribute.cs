        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext) {
            ...
            if (value is IEnumerable enumerable)
            {
                foreach (var item in enumerable)
                {
                    if (item == null)
                    {
                        results.Add(new ValidationResult("Encountered null object.", new []{validationContext.MemberName}));
                        continue;
                    }
            ...
            return results.Any() 
                ? new ValidationResult(string.Join(',', results.Select(result => result.ErrorMessage)), results.SelectMany(result => result.MemberNames)) 
